...
#*************************
label_node_docker_engine() {
  sed -i 's|#DOCKER_OPTS="--dns 8.8.8.8 --dns 8.8.4.4"|DOCKER_OPTS="--label=wn=true"|g' /etc/default/docker
...
# docker_run_portainer_agent_wn
#******************************
docker_run_portainer_agent_wn() {
  docker run -d \
   --name portainer-agent-wn \
   --network portainer-network \
   --publish $PORTAINER_AGENT_PORT:$PORTAINER_AGENT_PORT \
  ...
  docker inspect --format '{{.State.Status}}' portainer-agent-wn >> /tmp/portainer-agent-wn-output.txt
  grep running < /tmp/portainer-agent-wn-output.txt
  ...
  rm /tmp/portainer-agent-wn-output.txt
}
